...
- `EDITOR`：编辑者，可上传、删除文档，可搜索
- `VIEWER`：查看者，只读权限，可搜索

##### 2.2.2 权限矩阵

| 操作 | OWNER | EDITOR | VIEWER | 说明 |
|-----|-------|--------|--------|------|
| 查看知识库信息 | ✓ | ✓ | ✓ | 基础信息、统计数据 |
| 搜索文档 | ✓ | ✓ | ✓ | 向量搜索、全文搜索 |
| 查看文档列表 | ✓ | ✓ | ✓ | 含分页、筛选 |
...
```
KnowledgeBasePermission 实体：
├── id (UUID)
├── knowledge_base_id (外键)
├── user_id (被授权用户)
├── role (OWNER/EDITOR/VIEWER)
├── granted_by (授权人user_id)
├── granted_at (授权时间)
...
```
请求 → 身份认证(JWT) → 获取user_id → 查询权限表 → 验证操作权限 → 执行业务逻辑
```
