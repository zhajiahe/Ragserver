...
class TestRecursiveCharacterChunker:
    ...
    async def test_custom_separators(self, short_text: str):
        """测试自定义分隔符"""
        chunker = RecursiveCharacterChunker(chunk_size=300, chunk_overlap=50, separators=["\n\n", "\n", "。", ".", " "])
    ...
    async def test_example_file(self, example_text: str):
        """测试示例文件"""
        chunker = RecursiveCharacterChunker(chunk_size=800, chunk_overlap=200)
...
class TestBm25TextChunker:
    ...
    async def test_basic_chunking(self, short_text: str):
        """测试基本分块功能"""
        chunker = Bm25TextChunker(chunk_size=300, chunk_overlap=50, similarity_threshold=0.3)
    ...
    async def test_similarity_threshold(self, short_text: str):
        """测试相似度阈值"""
        # 高阈值应该产生更多块
        chunker_high = Bm25TextChunker(chunk_size=500, chunk_overlap=50, similarity_threshold=0.8)

        # 低阈值应该产生更少块
        chunker_low = Bm25TextChunker(chunk_size=500, chunk_overlap=50, similarity_threshold=0.1)
    ...
    async def test_example_file(self, example_text: str):
        """测试示例文件"""
        chunker = Bm25TextChunker(chunk_size=800, chunk_overlap=200, similarity_threshold=0.3)
...
class TestSemanticChunker:
    ...
    async def test_min_max_chunk_size(self, short_text: str):
        """测试最小/最大块大小"""
        chunker = SemanticChunker(similarity_threshold=0.7, min_chunk_size=50, max_chunk_size=300, chunk_overlap=20)
...
class TestChunkerFactory:
    ...
    async def test_create_recursive_chunker(self):
        """测试创建递归分块器"""
        config = {"max_chunk_size": 500, "chunk_overlap": 100, "separators": ["\n\n", "\n"]}
    ...
    async def test_create_bm25_chunker(self):
        """测试创建BM25分块器"""
        config = {"max_chunk_size": 600, "chunk_overlap": 150, "similarity_threshold": 0.4}
...
class TestChunkTextFunction:
    ...
    async def test_chunk_text_recursive(self, short_text: str):
        """测试递归分块"""
        config = {"strategy_type": "recursive", "config": {"max_chunk_size": 300, "chunk_overlap": 50}}
    ...
    async def test_chunk_text_bm25(self, short_text: str):
        ...
        config = {
            "strategy_type": "bm25",
            "config": {"max_chunk_size": 400, "chunk_overlap": 80, "similarity_threshold": 0.3},
        }
    ...
    async def test_default_strategy(self, short_text: str):
        """测试默认策略"""
        config = {"config": {"max_chunk_size": 300, "chunk_overlap": 50}}
...
class TestMinChunkSize:
    ...
    async def test_recursive_min_size(self, short_text: str):
        ...
        chunker = RecursiveCharacterChunker(chunk_size=500, chunk_overlap=50, min_chunk_size=min_size)
    ...
    async def test_bm25_min_size(self, short_text: str):
        ...
        chunker = Bm25TextChunker(chunk_size=500, chunk_overlap=50, min_chunk_size=min_size, similarity_threshold=0.3)
    ...
    async def test_factory_min_size(self, short_text: str):
        ...
        config = {
            ...
            "config": {"max_chunk_size": 500, "chunk_overlap": 50, "min_chunk_size": min_size},
        }
    ...
    async def test_different_min_sizes(self):
        ...
        for min_size in min_sizes:
            chunker = RecursiveCharacterChunker(chunk_size=200, chunk_overlap=20, min_chunk_size=min_size)
...
class TestCompareStrategies:
    ...
    async def test_compare_all_strategies(self, example_text: str):
        """比较三种策略的分块结果"""
        base_config = {"max_chunk_size": 800, "chunk_overlap": 200}

        # 递归分块
        config_recursive = {"strategy_type": "recursive", "config": base_config}
        chunks_recursive = await chunk_text(example_text, config_recursive)

        # BM25分块
        config_bm25 = {"strategy_type": "bm25", "config": {**base_config, "similarity_threshold": 0.3}}
        ...
        min_len = min(len(c["content"]) for c in chunks_recursive)
        max_len = max(len(c["content"]) for c in chunks_recursive)
        print(f"   最小/最大: {min_len} / {max_len}")

        print("\n2. BM25语义分块:")
        print(f"   块数: {len(chunks_bm25)}")
        print(f"   平均块大小: {sum(len(c['content']) for c in chunks_bm25) / len(chunks_bm25):.0f}")
        min_len_bm25 = min(len(c["content"]) for c in chunks_bm25)
        max_len_bm25 = max(len(c["content"]) for c in chunks_bm25)
        print(f"   最小/最大: {min_len_bm25} / {max_len_bm25}")

        print("\n3. 向量语义分块:")
        print(f"   块数: {len(chunks_semantic)}")
        print(f"   平均块大小: {sum(len(c['content']) for c in chunks_semantic) / len(chunks_semantic):.0f}")
        min_len_semantic = min(len(c["content"]) for c in chunks_semantic)
        max_len_semantic = max(len(c["content"]) for c in chunks_semantic)
        print(f"   最小/最大: {min_len_semantic} / {max_len_semantic}")
