...
from ragserver.app.dependencies.db import async_engine  # noqa: E402
from ragserver.app.models import Base  # noqa: E402
from ragserver.app.utils.logging_config import logger  # noqa: E402
from ragserver.config import settings  # noqa: E402
...
示例:
  ...
  python tests/reset_db.py --reset --force
    ...
    parser.add_argument("--info", action="store_true", help="显示数据库信息（不执行重置）")

    parser.add_argument("--reset", action="store_true", help="重置数据库（删除所有表并重新创建）")

    parser.add_argument("--force", action="store_true", help="强制重置（跳过确认，危险！仅用于自动化脚本）")
