...
from ragserver.app.models import Collection, Document, User
...
class TestDocumentProcessing:
    ...
    async def test_process_documents_success(
        self, async_client: AsyncClient, auth_headers: dict, test_documents: list[Document]
    ):
        """测试成功处理文档"""
        payload = {"document_ids": [str(doc.id) for doc in test_documents]}

        res = await async_client.post("/api/v1/documents/process", json=payload, headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_process_documents_empty_list(self, async_client: AsyncClient, auth_headers: dict):
        """测试空文档列表"""
    ...
    async def test_reprocess_documents_success(
        ...
        res = await async_client.post("/api/v1/documents/reprocess", json=payload, headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_reprocess_documents_unauthorized(self, async_client: AsyncClient, test_documents: list[Document]):
        """测试未认证重新处理"""
        payload = {"document_ids": [str(doc.id) for doc in test_documents]}

        res = await async_client.post("/api/v1/documents/reprocess", json=payload)
