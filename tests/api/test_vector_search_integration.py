...
class TestVectorSearchMultipleCollections:
    ...
    async def test_search_multiple_collections(
        ...
        payload = {"query": "人工智能", "top_k": 5, "threshold": 0.3, "collection_ids": [str(first_collection)]}
...
class TestSearchEdgeCases:
    ...
    async def test_search_no_chunks_in_collection(
        ...
        payload = {"query": "测试查询", "top_k": 5, "threshold": 0.3, "collection_ids": [str(empty_collection.id)]}
    ...
    async def test_search_nonexistent_collection(
        ...
        """测试搜索不存在的知识库"""
        fake_id = str(uuid4())
        payload = {"query": "测试查询", "top_k": 5, "collection_ids": [fake_id]}
