...
class BaseChunker(ABC):
    ...
    def _format_chunks(self, chunks: list[str]) -> list[dict[str, Any]]:
        ...
        for chunk in chunks:
            ...
            formatted_chunks.append(
                {
                    "content": chunk,
                    "metadata": {"chunk_index": chunk_index, "chunk_size": len(chunk), "char_count": len(chunk)},
                }
            )
            chunk_index += 1
        ...
        return (
            ...
            if formatted_chunks
            else [
                {
                    ...
                    "metadata": {"chunk_index": 0, "chunk_size": 0, "char_count": 0},
                }
            ]
        )
    ...
    def _add_overlap(self, chunks: list[str]) -> list[str]:
        ...
        for i in range(1, len(chunks)):
            # 从前一个块的末尾提取重叠部分
            overlap_text = self._extract_overlap_text(chunks[i - 1], self.chunk_overlap)
...
class Bm25TextChunker(BaseChunker):
    ...
    def _calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        try:
            ...
            vectorizer = TfidfVectorizer(token_pattern=r"(?u)\b\w+\b", min_df=1, max_df=1.0)
...
class SemanticChunker(BaseChunker):
    ...
    def _identify_breakpoints(self, similarities: list[float], paragraphs: list[str]) -> list[int]:
        """识别语义边界点
    ...
    def _create_chunks(self, paragraphs: list[str], breakpoints: list[int]) -> list[str]:
        """根据边界点创建文本块"""
        chunks = []
        start_idx = 0
    ...
    def create_chunker(strategy_type: str, config: dict[str, Any]) -> BaseChunker:
        ...
        if strategy_type == "recursive":
            ...
            return RecursiveCharacterChunker(
                chunk_size=chunk_size, chunk_overlap=chunk_overlap, min_chunk_size=min_chunk_size, separators=separators
            )
