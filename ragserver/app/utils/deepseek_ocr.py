...
class DocumentOCRParser:
    ...
    def __init__(
        self, model: str = "deepseek-ai/DeepSeek-OCR", dpi: int = 144, max_concurrent: int = 30, timeout: int = 300
    ):
        """初始化解析器
    ...
    async def parse_document(self, file_path: str, extract_images: bool = False, output_dir: str | None = None) -> str:
        ...
        try:
            ...
            # 4. 后处理并生成 Markdown
            markdown = await self._post_process(results, extract_images=extract_images, output_dir=output_dir)
    ...
    def _get_file_extension(self, url: str, content_type: str | None = None) -> str:
    ...
    async def _process_single_image(self, page_idx: int, image: Image.Image) -> tuple[int, str]:
        """使用 LLM 处理单张图片"""
        async with self.semaphore:
            try:
                image_base64 = await asyncio.to_thread(self._image_to_base64, image, "PNG")
    ...
    async def _process_images_with_llm(self, images: list[Image.Image]) -> list[tuple[int, str]]:
        """并发处理所有图片"""
        tasks = [self._process_single_image(idx, image) for idx, image in enumerate(images)]
    ...
    def _clean_content(
        self, content: str, page_idx: int, extract_images: bool = False, output_dir: str | None = None
    ) -> str:
        """清理内容"""
        # 提取图片标记
    ...
    async def _post_process(
        self, results: list[tuple[int, str]], extract_images: bool = False, output_dir: str | None = None
    ) -> str:
        ...
        for page_idx, content in results:
            ...
            cleaned_content = self._clean_content(
                content, page_idx, extract_images=extract_images, output_dir=output_dir
            )
...
async def parse_document_to_markdown(
    ...
    Example:
        ...
        ```
    """
    parser = DocumentOCRParser(model=model, dpi=dpi, max_concurrent=max_concurrent, timeout=timeout)

    return await parser.parse_document(file_path=file_path, extract_images=extract_images, output_dir=output_dir)
