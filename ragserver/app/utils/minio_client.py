...
class AsyncMinioClient:
    ...
    async def upload_file(
        ...
        async with self._get_client() as client:
            ...
            if file_existed:
                ...
                await client.upload_fileobj(file_obj, bucket_name, object_key, ExtraArgs=extra_args)
                logger.info(
                    f"Uploaded {file_name} (MD5: {md5_hash}, SHA256: {sha256_hash}) to {bucket_name}/{object_key}"
                )
    ...
    async def get_file_by_md5(self, bucket_name: str, md5_hash: str, extension: str = "") -> dict[str, any] | None:
        """通过 MD5 获取文件信息和下载路径
    ...
    async def download_file_by_md5(self, bucket_name: str, md5_hash: str, extension: str = ""):
        """通过 MD5 下载文件"""
        # 确保初始化
        await self._initialize_once()
    ...
    async def delete_file_by_md5(self, bucket_name: str, md5_hash: str, extension: str = ""):
        """通过 MD5 删除文件"""
        # 确保初始化
        await self._initialize_once()
    ...
    async def generate_presigned_url(
        self, bucket_name: str, md5_hash: str, extension: str = "", expiration: int = 3600
    ) -> str:
        """生成预签名 URL（用于临时访问）
