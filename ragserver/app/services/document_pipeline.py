...
from ragserver.app.models import Collection, Document, DocumentChunk
...
class DocumentProcessingPipeline:
    ...
    async def process_document(self, document_id: UUID) -> dict[str, Any]:
        ...
        try:
            ...
            # 2. 获取知识库信息
            result = await self.db.execute(select(Collection).where(Collection.id == document.collection_id))
            ...
            response = await minio_client.download_file(settings.minio_bucket_documents, object_key)
            ...
            content_text = await parse_document_content(file_content, document.file_type, document.filename)
            ...
            await self._save_chunks(
                chunks=chunks, embeddings=all_embeddings, document=document, chunking_config=chunking_config
            )
    ...
    async def _generate_embeddings(self, chunks: list[str], document: Document) -> list[list[float]]:
        """生成向量（批量处理）
    ...
    async def _save_chunks(
        self, chunks: list[str], embeddings: list[list[float]], document: Document, chunking_config: dict[str, Any]
    ) -> None:
        """保存分块到数据库
    ...
    def _get_chunking_config(self, collection: Collection, document: Document | None = None) -> dict[str, Any]:
        """获取分块配置（文档级优先，知识库级备选）
