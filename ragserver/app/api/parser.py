...
from ragserver.app.models import Document, User
...
@router.post("/process", status_code=status.HTTP_202_ACCEPTED, summary="处理文档（批量）")
    ...
    for doc_id in request.document_ids:
        try:
            ...
            if document.status == "completed":
                # 已经处理完成的文档跳过
                results.append({"document_id": str(doc_id), "status": "skipped", "message": "文档已处理完成"})
                continue
...
@router.post("/reprocess", status_code=status.HTTP_202_ACCEPTED, summary="重新处理文档")
