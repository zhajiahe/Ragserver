...
class CollectionBase(BaseModel):
    ...
    icon_url: str | None = Field(None, max_length=500, description="图标URL")
    language: str = Field(default="zh", max_length=20, description="语言")
    settings: dict | None = Field(default_factory=dict, description="知识库配置")
...
class CollectionUpdate(BaseModel):
    ...
    name: str | None = Field(None, min_length=1, max_length=100, description="知识库名称")
    description: str | None = Field(None, max_length=500, description="知识库描述")
    icon_url: str | None = Field(None, max_length=500, description="图标URL")
    language: str | None = Field(None, max_length=20, description="语言")
    settings: dict | None = Field(None, description="知识库配置")
...
class CollectionShareCreate(BaseModel):
    ...
    description: str | None = Field(None, max_length=500, description="分享描述")
    expires_in_days: int | None = Field(None, ge=1, le=365, description="过期天数（不设置则永久有效）")
    search_config: dict | None = Field(default_factory=dict, description="搜索配置")
...
async def list_collections(
    status: str | None = Query(None, description="状态筛选: active/archived"),
...
async def create_collection(
    req: CollectionCreate, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """创建知识库
    ...
    """
    # 检查用户知识库数量限制
    count_query = select(func.count()).select_from(Collection).where(Collection.user_id == current_user.id)
    ...
    # 检查知识库名称是否重复（同一用户下）
    existing_query = select(Collection).where(Collection.user_id == current_user.id, Collection.name == req.name)
...
async def get_collection(
    collection_id: UUID, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """获取知识库详情
...
async def update_collection(
    ...
    if collection.status == "archived":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="无法更新已归档的知识库")

    # 如果更新名称，检查是否重复
    if req.name and req.name != collection.name:
        existing_query = select(Collection).where(
            Collection.user_id == current_user.id, Collection.name == req.name, Collection.id != collection_id
        )
        existing_result = await db.execute(existing_query)
        if existing_result.scalar_one_or_none():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="知识库名称已存在")
...
async def delete_collection(
    ...
    # 检查状态
    if collection.status == "archived":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="无法删除已归档的知识库，请先取消归档")
...
async def archive_collection(
    ...
    # 检查状态
    if collection.status == "archived":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="知识库已经是归档状态")
...
async def list_shares(
    ...
    # 查询分享列表
    shares_query = select(CollectionShare).where(CollectionShare.collection_id == collection_id)
...
class CollectionShareUpdate(BaseModel):
    ...
    name: str | None = Field(None, min_length=1, max_length=100, description="分享名称")
    description: str | None = Field(None, max_length=500, description="分享描述")
    is_active: bool | None = Field(None, description="是否启用")
    expires_in_days: int | None = Field(None, ge=1, le=365, description="从现在起过期天数")
    search_config: dict | None = Field(None, description="搜索配置")


@shares_router.get("/{share_id}", response_model=CollectionShareResponse)
async def get_share(
    share_id: UUID, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """获取分享详情

    - 只能查看自己创建的分享
    """
    query = select(CollectionShare).where(CollectionShare.id == share_id, CollectionShare.created_by == current_user.id)
    result = await db.execute(query)
