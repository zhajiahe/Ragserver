...
class DocumentBase(BaseModel):
    ...
    meta: dict | None = Field(default_factory=dict, description="元数据")
...
class DocumentUpdate(BaseModel):
    ...
    chunking_config: dict | None = Field(None, description="分块配置")
    meta: dict | None = Field(None, description="元数据")
...
async def verify_collection_access(db: AsyncSession, collection_id: UUID, user_id: UUID) -> Collection:
    """验证用户是否有权限访问知识库"""
...
async def verify_document_access(db: AsyncSession, document_id: UUID, user_id: UUID) -> Document:
    """验证用户是否有权限访问文档"""
    result = await db.execute(select(Document).where(Document.id == document_id, Document.uploaded_by == user_id))
...
@router.get(
    "/api/v1/collections/{collection_id}/documents", response_model=DocumentListResponse, summary="获取知识库文档列表"
)
async def get_documents(
    collection_id: UUID,
    status_filter: str | None = Query(None, description="按状态过滤: pending/processing/completed/failed"),
    ...
    # 构建查询
    query = select(Document).where(Document.collection_id == collection_id, Document.uploaded_by == current_user.id)
...
@router.get("/api/v1/documents/{document_id}", response_model=DocumentResponse, summary="获取文档详情")
...
@router.delete("/api/v1/documents", status_code=status.HTTP_204_NO_CONTENT, summary="删除文档（批量）")
async def delete_documents(
    ...
    # TODO: 删除 MinIO 中的文件
    await db.execute(
        delete(Document).where(Document.id.in_(request.document_ids), Document.uploaded_by == current_user.id)
    )
...
@router.put("/api/v1/documents/{document_id}", response_model=DocumentResponse, summary="更新文档解析配置")
...
@router.get("/api/v1/documents/{document_id}/status", response_model=DocumentStatusResponse, summary="查询文档处理状态")
...
@router.get(
    "/api/v1/documents-chunks/{document_id}", response_model=list[DocumentChunkResponse], summary="获取文档分块列表"
)
async def get_document_chunks(
    ...
    result = await db.execute(
        ...
        .where(DocumentChunk.document_id == document_id, DocumentChunk.user_id == current_user.id)
