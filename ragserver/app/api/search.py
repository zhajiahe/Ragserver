...
from ragserver.app.models import CollectionShare, DocumentChunk, User
...
class SearchRequest(BaseModel):
    ...
    collection_ids: list[UUID] | None = Field(default=None, description="知识库ID列表（可选）")
...
async def vector_search(
    ...
    # 构建查询
    query_stmt = select(
        DocumentChunk, (1 - DocumentChunk.content_embedding.cosine_distance(query_embedding)).label("similarity")
    ).where((1 - DocumentChunk.content_embedding.cosine_distance(query_embedding)) >= threshold)
    ...
    # 排序和限制
    query_stmt = query_stmt.order_by(DocumentChunk.content_embedding.cosine_distance(query_embedding)).limit(top_k)
...
async def fulltext_search(
    db: AsyncSession, query: str, user_id: UUID | None, collection_ids: list[UUID] | None, top_k: int
) -> list[tuple]:
    """全文搜索 (ParadeDB BM25)
    ...
    for row in rows:
        ...
        chunk_result = await db.execute(select(DocumentChunk).where(DocumentChunk.id == row.id))
...
async def hybrid_search(
    ...
    """
    # 1. 执行向量搜索
    vector_results = await vector_search(db, query, user_id, collection_ids, top_k * 2, threshold)

    # 2. 执行全文搜索
    fulltext_results = await fulltext_search(db, query, user_id, collection_ids, top_k * 2)
    ...
    for chunk, similarity in vector_results:
        chunk_scores[chunk.id] = {"chunk": chunk, "vector_score": float(similarity), "fulltext_score": 0.0}
    ...
    for chunk, score in fulltext_results:
        if chunk.id in chunk_scores:
            ...
            chunk_scores[chunk.id] = {"chunk": chunk, "vector_score": 0.0, "fulltext_score": float(score)}
...
async def search(
    req: SearchRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """搜索知识库
    ...
    try:
        ...
        if req.mode == SearchMode.VECTOR:
            rows = await vector_search(db, req.query, current_user.id, req.collection_ids, req.top_k, req.threshold)
        elif req.mode == SearchMode.FULLTEXT:
            rows = await fulltext_search(db, req.query, current_user.id, req.collection_ids, req.top_k)
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"不支持的搜索模式: {req.mode}")
...
async def search_by_share_token(share_token: str, req: SearchRequest, db: AsyncSession = Depends(get_db)):
    """通过分享链接搜索知识库
    ...
    # 查询分享链接
    share_query = select(CollectionShare).where(CollectionShare.share_token == share_token)
    ...
    if not share.is_active:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="分享链接已停用")

    # 检查是否过期
    if share.expires_at and share.expires_at < get_current_time():
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="分享链接已过期")
    ...
    try:
        ...
        if req.mode == SearchMode.VECTOR:
            rows = await vector_search(db, req.query, None, collection_ids, req.top_k, req.threshold)
        elif req.mode == SearchMode.FULLTEXT:
            rows = await fulltext_search(db, req.query, None, collection_ids, req.top_k)
        elif req.mode == SearchMode.HYBRID:
            rows = await hybrid_search(
                db, req.query, None, collection_ids, req.top_k, req.threshold, req.vector_weight, req.fulltext_weight
            )
        else:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"不支持的搜索模式: {req.mode}")
