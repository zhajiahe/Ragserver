...
class Settings(BaseSettings):
    ...
    redis_password: str | None = Field(default="ragserver_redis_password", description="Redis 密码")
    ...
    # ==================== JWT 认证配置 ====================
    jwt_secret_key: str = Field(default_factory=lambda: secrets.token_urlsafe(32), description="JWT 密钥")
    ...
    allowed_file_types: list[str] = Field(
        default=["pdf", "docx", "doc", "txt", "md", "html", "htm", "xlsx", "xls", "csv", "pptx", "jpg", "png"],
        description="允许的文件类型",
    )
    ...
    siliconflow_api_base: str = Field(default="https://api.siliconflow.cn/v1", description="硅基流动 API Base URL")
...
def validate_settings() -> None:
    ...
    if settings.default_search_type not in ["vector", "fulltext", "hybrid"]:
        errors.append(
            f"DEFAULT_SEARCH_TYPE 必须是 'vector'、'fulltext' 或 'hybrid'，当前值: {settings.default_search_type}"
        )
