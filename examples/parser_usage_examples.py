...
async def example_6_batch_parsing():
    ...
    # 并行解析
    tasks = [
        parse_document(content, file_type, filename)
        for content, file_type, filename in documents
    ]
...
async def example_9_encoding_detection():
    ...
    # UTF-8 编码
    utf8_content = "你好，世界！".encode('utf-8')
...
async def example_10_performance_test():
    ...
    start_time = time.time()
    tasks = [
        parse_document(large_text, "txt", f"large_{i}.txt")
        for i in range(5)
    ]
